...
def download_image(img_url, session):
    ...
    if (img_url.startswith("https://cdn.onepiecechapters.com/file/") and
        ("onepiece" in img_url or "op_tcb" in img_url) and
...
one_piece_folder = Path(filedialog.askdirectory(title="Select the path where you want to create the 'One Piece' folder"))

# Create a folder named "One Piece" if it doesn't exist
one_piece_folder = one_piece_folder / "One Piece"
one_piece_folder.mkdir(exist_ok=True)

# Set the URL of the main page
main_url = "https://tcbscans.com/mangas/5/one-piece"
...
# Find all links to chapters that have "one-piece-chapter" in them
chapter_links = main_soup.find_all("a", href=lambda href: href and "one-piece-chapter" in href)

# Iterate over the chapter links
for link in chapter_links:
    # Get the chapter number from the link
    chapter_number = link["href"].split("one-piece-chapter-")[1].split("-")[0]

    # Check if a folder for the chapter already exists
    chapter_folder = one_piece_folder / chapter_number
    ...
    chapter_page = session.get(f"https://tcbscans.com{link['href']}")
